...
import { ScrollSlides } from "@/components/ui/scroll-slides";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function Ecosistema() {
  ...
  const slides = [
    ...
    <div className="container mx-auto grid items-center gap-8 md:grid-cols-2 md:gap-12">
      ...
      <GlassCard className="md:p-10">
        <p className="text-lg leading-relaxed text-muted-foreground">
          ...
          finanziamenti: coordiniamo competenze specialistiche per trasformare progetti innovativi
    ...
    <div className="container mx-auto max-w-3xl text-center">
      <span className="text-xs uppercase tracking-[0.3em] text-violet-glow">Un modello integrato</span>
      ...
      <p className="mt-6 text-lg leading-relaxed text-muted-foreground">
        Abbiamo costruito una rete di competenze specialistiche nella ricerca pubblica e privata,
        integrata con una struttura dedicata al trasferimento tecnologico. Affianchiamo le imprese
        in ogni fase del percorso di innovazione, dalla definizione dell'idea alla sua
    ...
    <div className="container mx-auto">
      <div className="mx-auto mb-7 max-w-3xl text-center">
        ...
        <h2 className="mt-3 text-3xl font-bold text-gradient md:text-4xl">Dal laboratorio al mercato</h2>
        ...
        {focus.map((f) => (
          ...
          >
            <GlassCard className="flex h-full flex-col transition-transform duration-300 group-hover:-translate-y-1">
              <f.icon className="mb-3 text-violet-glow" size={26} />
              <h3 className="font-display text-base font-semibold leading-tight">{f.title}</h3>
              <p className="mt-2 flex-1 text-xs text-muted-foreground">{f.desc}</p>
              <span className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-petrol dark:text-teal-300 group-hover:underline group-hover:underline-offset-4">
                <BookOpen size={13} />
                Leggi di più
              </span>
            </GlassCard>
    ...
    <div className="container mx-auto max-w-4xl">
      <div className="glass-petrol relative overflow-hidden rounded-[2.5rem] p-10 text-center md:p-16">
        <blockquote className="font-display text-2xl font-medium leading-snug text-white md:text-3xl">
          "Le imprese non hanno bisogno di dieci fornitori diversi per innovare. Hanno bisogno di un
          partner capace di coordinare{" "}
  ...
  return (
    <PageShell>
      <ScrollSlides slides={slides} />

      {/* Modale flash-card (invariato) */}
